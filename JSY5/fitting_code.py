...
for r in range(Nr):
    ...
    if r==0:
            p = lmfit.Parameters()
            p.add_many(('nc', 0.9,True,0.8,1),('ns', 0,True,0,0.2),('nh', 0.1,True,0,0.2), ('Tc_pal', 10*10**5,True,1*10**5,10*10**5), ('Tc_per', 10*10**5,True,1*10**5,10*10**5), ('Ts_pal', 10*10**5,True,1*10**5,20*10**5), ('Ts_per', 10*10**5,True,1*10**5,20*10**5),('Th_pal', 10*10**5,True,1*10**5,10*10**5), ('Th_per', 10*10**5,True,1*10**5,10*10**5), ('Uc',0,True,-1.5,0),('Us',0,True,0,17),('Uh',0,True,-1,1),('kappah',4,True,2,6))
    else:                          #,('Us',0,True,0,1.5) , ('Uc',0,True,-0.4,0) 
            p = lmfit.Parameters()
            p.add_many(('nc', nc[r-1],True,0.8,1),('ns', ns[r-1],True,0,0.2),('nh', nh[r-1],True,0,0.2), ('Tc_pal', Tc_pal[r-1],True,1*10**5,10*10**5), ('Tc_per', Tc_per[r-1],True,1*10**5,10*10**5), ('Ts_pal', Ts_pal[r-1],True,1*10**5,20*10**5), ('Ts_per', Ts_per[r-1],True,1*10**5,20*10**5),('Th_pal', Th_pal[r-1],True,1*10**5,10*10**5), ('Th_per', Th_per[r-1],True,1*10**5,10*10**5), ('Uc',Uc[r-1],True,-1.5,0),('Us',Us[r-1],True,0,17),('Uh',Uh[r-1],True,-1,1),('kappah',kappah[r-1],True,2,6))
    ...
    for j in range(Nv):
        for i in range(Nv):
            fitting[j*Nv+i]=(nh[r])*(r_s**3)*Density[r]*(v_th_function(Th_pal[r])*v_th_function(Th_per[r])**2)**(-1)*(2/(np.pi*(2*kappah[r]-3)))**1.5*(gamma(kappah[r]+1)/gamma(kappah[r]-0.5))*(1.+(2/(2*kappah[r]-3))*(((per_v[j])/v_th_function(Th_per[r]))**2)+(2/(2*kappah[r]-3))*(((pal_v[i]-Uh[r])/v_th_function(Th_pal[r]))**2))**(-kappah[r]-1.)+nc[r]*(r_s**3)*Density[r]*(v_th_function(Tc_pal[r])*v_th_function(Tc_per[r])**2)**(-1)*(2/(np.pi*(2*20-3)))**1.5*(gamma(20+1)/gamma(20-0.5))*(1.+(2/(2*20-3))*(((per_v[j])/v_th_function(Tc_per[r]))**2)+(2/(2*20-3))*(((pal_v[i]-Uc[r])/v_th_function(Tc_pal[r]))**2))**(-20-1.)+(ns[r])*(r_s**3)*Density[r]*(v_th_function(Ts_pal[r])*v_th_function(Ts_per[r])**2)**(-1)*(2/(np.pi*(2*20-3)))**1.5*(gamma(20+1)/gamma(20-0.5))*(1.+(2/(2*20-3))*(((per_v[j])/v_th_function(Ts_per[r]))**2)+(2/(2*20-3))*(((pal_v[i]-Us[r])/v_th_function(Ts_pal[r]))**2))**(-20-1.)
    
    fitting_c=np.zeros(shape = (Nv**2, 1))
    for j in range(Nv):
        for i in range(Nv):
            fitting_c[j*Nv+i]=nc[r]*(r_s**3)*Density[r]*(v_th_function(Tc_pal[r])*v_th_function(Tc_per[r])**2)**(-1)*(2/(np.pi*(2*20-3)))**1.5*(gamma(20+1)/gamma(20-0.5))*(1.+(2/(2*20-3))*(((per_v[j])/v_th_function(Tc_per[r]))**2)+(2/(2*20-3))*(((pal_v[i]-Uc[r])/v_th_function(Tc_pal[r]))**2))**(-20-1.)

    fitting_s=np.zeros(shape = (Nv**2, 1))
    for j in range(Nv):
        for i in range(Nv):
            fitting_s[j*Nv+i]=(ns[r])*(r_s**3)*Density[r]*(v_th_function(Ts_pal[r])*v_th_function(Ts_per[r])**2)**(-1)*(2/(np.pi*(2*20-3)))**1.5*(gamma(20+1)/gamma(20-0.5))*(1.+(2/(2*20-3))*(((per_v[j])/v_th_function(Ts_per[r]))**2)+(2/(2*20-3))*(((pal_v[i]-Us[r])/v_th_function(Ts_pal[r]))**2))**(-20-1.)

    fitting_h=np.zeros(shape = (Nv**2, 1))
    for j in range(Nv):
        for i in range(Nv):
            fitting_h[j*Nv+i]=(nh[r])*(r_s**3)*Density[r]*(v_th_function(Th_pal[r])*v_th_function(Th_per[r])**2)**(-1)*(2/(np.pi*(2*kappah[r]-3)))**1.5*(gamma(kappah[r]+1)/gamma(kappah[r]-0.5))*(1.+(2/(2*kappah[r]-3))*(((per_v[j])/v_th_function(Th_per[r]))**2)+(2/(2*kappah[r]-3))*(((pal_v[i]-Uh[r])/v_th_function(Th_pal[r]))**2))**(-kappah[r]-1.)
...
ax.plot(z,Uh,linewidth=4.0, color='g',label=r'$U_h/v_{Ae0}$');
plt.legend(loc='upper right')
...
ax.plot(z,kappah,linewidth=4.0, color='g',label=r'$kappa_h$');
plt.legend(loc='upper right')
